...
class jspider(CrawlSpider):
	...
	def parse(self,response):
		...
		for cite_data in cite_datas:
			...
			try:	
				journal_detail['year'] = remove_tags(html.tostring(cite_name.xpath("//span[@class='pubYear']")[0])).replace(",","").strip()
			except:
				journal_detail['year'] = ""
			try:
		...
		try:
			...
			for rel_data in rel_datas:
				...
				if "Pages" in jour_details[3]:
					...
					jour_det['pages'] = ""
				jour_det['pub_info'] = jour_details[2]
